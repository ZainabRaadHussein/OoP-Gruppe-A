...
class MainWindow(QMainWindow):
    ...
    def update_plot(self):
        ...
        colors = ['r', 'g', 'orange']
        bars = BarGraphItem(x=[0, 1, 2], height=values, width=0.6, brushes=colors)
        self.bar_widget.addItem(bars)
        for i, val in enumerate(values):
            label = ['Max', 'Min', 'Ø'][i]
            text = TextItem(f"{label}: {val:.1f}", anchor=(0.5, -1.0), color=colors[i])
            self.bar_widget.addItem(text)
            text.setPos(i, val)
        self.trend_plot.clear()
        self.trend_label.setText("")
        self.trend_plot.plot(self.x_data, self.y_data, pen='g')
    def export_csv(self):
